## Entity

reagent-dispenser-component-activate-no-hands = У вас нема рук.
reagent-dispenser-component-cannot-put-entity-message = Ви не можете вставити це в дозатор!

## Bound UI

reagent-dispenser-bound-user-interface-title = Дозатор Реагентів

## UI

reagent-dispenser-window-amount-to-dispense-label = Кількість
reagent-dispenser-window-container-label = Контейнер:
reagent-dispenser-window-clear-button = Очистити
reagent-dispenser-window-eject-button = Вийняти
reagent-dispenser-window-no-container-loaded-text = Контейнера не вставлено.
reagent-dispenser-window-reagent-name-not-found-text = Назва реагенту не знайдена
reagent-dispenser-window-unknown-reagent-text = Невідомий реагент
reagent-dispenser-window-quantity-label-text = {$quantity}ю
